...
def _is_wiki_sparse(wiki_dir, min_business_files=3):
    ...
    导致 evidence_verify 过度剔除合理 items(2026-04-24 sample module模板 PRD
    实测 5/7 条 A 类被撤)。
...
def verify_evidence(items, workspace, client=None, wiki_pages=None, prd_content=None):
    ...
    # 之前逻辑在 workspace-sample 这种无 wiki 上下文场景下 100% retract A 类,
        ...
        if ev_type == "A":
            if wiki_sparse:
                ...
                if not _found:
                    ...
                    # 会把合理 item 一并抹杀(sample module模板 PRD 实测 4 条 A 类被撤).
