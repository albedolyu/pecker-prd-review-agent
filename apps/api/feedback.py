"""
Pecker PRD 评审反馈闭环模块 — 信鸽 v1 (commit-watch 模式)
...
def _collect_commit_message_signals(code_dir, scope_keywords=None, since_days=30):
    ...
    这是把"Pecker评审"和"AI Coding 真实交付"接通的最后一公里。
    """
    signals = []
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pecker PRD 评审反馈闭环 — 信鸽 v1 [DEPRECATED, 用 scripts/feedback_v2.py]"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report",
        help="Pecker评审报告路径（如 output/PRD_改动报告_20260411.md）",
    )
    parser.add_argument(
        "--prd",
    ...
    parser.add_argument(
        "--prd-name",
        default=None,
        help="PRD 名称标识（用于规则历史记录中标注来源，如 'sample-case'）",
    )
