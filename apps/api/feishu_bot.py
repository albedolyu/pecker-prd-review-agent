"""
Pecker飞书 Bot -- 接收 PRD 文件，执行评审，发送交互式结果卡片
...
def create_app():
    ...
    app = FastAPI(title="Pecker飞书 Bot", version="1.0.0")
...
async def _handle_message(event):
    ...
    # 优先匹配反馈消息 — "@Pecker R-001 是误报" 不能掉到评审分支
...
def _build_progress_card(prd_name):
    ...
    return {
        "header": {
            "title": {"tag": "plain_text", "content": f"Pecker评审: {prd_name}"},
            "template": "blue",
        },
        "elements": [
...
def _build_result_card(items, prd_name, peck_score, workspace=""):
    ...
    return {
        "header": {
            "title": {"tag": "plain_text", "content": f"Pecker评审完成: {prd_name}"},
...
#
# 触发示例 (PM 在群里 @Pecker):
#   "@Pecker R-001 是误报, 分页字段已统一约定为 20"
#       → outcome=reject + auto-learning (累计 reject>=3 时)
#   "@Pecker R-005 接受"
#       → outcome=accept
#   "@Pecker RC-005 这条以后都按 chill profile 隐藏"
#       → outcome=reject + 显式 learning record
#   "@Pecker R-003 改写: 应该直接说排序规则需要补充章节"
#       → outcome=edit + reason
#
