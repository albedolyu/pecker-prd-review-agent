...
describe("supplemental materials", () => {
  ...
  it("redacts sensitive Figma link query params before adding review context", () => {
    const sensitive = "secret-" + "token";
    const material = buildFigmaRawMaterial(
      `https://www.figma.com/design/abc123/Foo?node-id=1-2&access_token=${sensitive}`,
      "PRD text",
    );

    expect(material).toContain("node_id: 1:2");
    expect(material).toContain("access_token=[REDACTED]");
    expect(material).not.toContain(sensitive);
  });
  ...
  it("redacts signed markdown image reference URLs before adding review context", () => {
    const sensitive = "secret-" + "signature";
    const material = buildImageReferenceRawMaterial({
      alt: "flow",
      url: `https://cdn.example/flow.png?sig=${sensitive}&version=1`,
      source: "PRD text",
    });
  ...
  it("summarizes image and Figma material counts for review context", () => {
    const rawMaterials = [
      buildImageRawMaterial({
        name: "flow.png",
        mimeType: "image/png",
