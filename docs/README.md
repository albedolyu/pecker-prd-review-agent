...
| [SPLIT_PLAN.md](SPLIT_PLAN.md) | 重构规划 | `parallel_review.py`（1833 行）按职责拆 6 模块的方案（config/prompting/worker/orchestration/evidence/merging） |
| [research/](research/) | 研究归档 | Claude Code v2.1.107 源码逆向的 3 轮研究报告（已落地到代码） |

## 工作流约定

- **主代码改动前先读对应诊断/规划**：如改 `parallel_review.py` 前读 `SPLIT_PLAN.md`；改 worker 错误处理前读 `STABILITY_DIAGNOSIS.md`
