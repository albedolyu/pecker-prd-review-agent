...
> 5 个任务预估 **3.0 天** (第二版修订后 T3 涨工),第一周收尾。
>
> **audit 探针派出后未产出** (task runner 异常,0 字节文件 35min 无动静, `TaskStop` 返回 no task) → 以现场 grep 定稿。行号基于 2026-04-24 main HEAD commit `b107856`。
>
> **第二版自检发现**:
> - API flow 和 CLI flow 的 pipeline **不同**: CLI (`run_session.py:276`) 走 full `verify_evidence`, API (`api/routes/review.py`) 仅靠 goshawk `_verify_wiki_evidence` 侧查。T3 funnel 分两条 flow 写。
...
### T3. Funnel stage emit — CLI flow 优先 (5 层), API flow 4 层并行

**优先级**: P1
**工量**: 1.5 天 (第二版从 1d 涨,因为要分两条 flow)
**依赖**: 无 (所有 emit 用 try/except,失败不阻塞)
**参考 spec**: `docs/review-funnel-schema.md`

**Flow 差异** (第二版新发现):
- **CLI** (`run_session.py`) : raw → dedup (parallel_review 内) → `verify_evidence` (L276) → goshawk → 无 PM 环节 (CLI 不走 Phase 3) ⇒ 4 层
- **API** (`api/routes/review.py`) : raw → dedup (parallel_review 内) → ~~verify_evidence 跳过~~ → goshawk (含 `_verify_wiki_evidence` 侧查) → PM (Phase 3 confirm) ⇒ 4 层
- **合并**: 两条都 4 层,但中间那层语义不同 (CLI=verify_evidence, API=goshawk 内侧查)

**修改文件 (API flow)**:
- `api/routes/review.py:371` 之后 → `funnel_stage_worker_raw` (from `worker_done` 事件累加) + `funnel_stage_after_dedup` (from `len(merged_items)`)
- `api/routes/review.py:411` 之后 → `funnel_stage_after_goshawk` + `delta_breakdown` + `facet_links`
  - 需要 `goshawk_advisor.py apply_advisor_result` 返回 `(items, telemetry)` 二元组,或把 `_funnel_telemetry` 塞到 `items[0]` 字段
...
**修改文件 (CLI flow)**:
- `run_session.py:276` 前后 → `funnel_stage_after_dedup` + `funnel_stage_after_evidence_verify`
  - `review/evidence_verify.py verify_evidence` 返回值 (目前 `(verified, failed, details)`) 扩展 telemetry dict: `retracted_by_reason` / `downgraded_by_reason` / `wiki_mode` / `authority_distribution`
- `run_session.py` goshawk 调用点之后 → `funnel_stage_after_goshawk`
- `run_session.py` 结束前 → `funnel_summary`

**验证命令**:
```bash
python run_session.py 未准入境需求文档-v1.0 --workspace workspace-侵权软件 \
    --non-interactive --auto-decide accept-all
grep -c "funnel_stage_" workspace-侵权软件/output/sessions/rev_*.jsonl   # 期望 ≥ 4
