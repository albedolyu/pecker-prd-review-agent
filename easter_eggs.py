...
FORTUNE_QUOTES = [
    ...
    "并行评审就像鸟群迁徙——方向一致，各走各的航线。—— 啄木鸟",
    "Tier 1 补完了吗？没有？那你还在看什么金句。—— 伯劳",
    "一份好的 CHANGELOG 比一百行注释更有价值。—— 鸮鹦",
]
...
ACHIEVEMENTS = {
    "first_peck": {
        ...
        "desc": "完成第一次 PRD 评审",
        "quote": "每只鸟都要迈出第一步",
        "detect": lambda stats, items, **kw: stats.get("total_reviews", 0) == 1,
    },
    "iron_beak": {
        "name": "铁嘴",
        "icon": "🦷",
        ...
        "detect": lambda stats, items, **kw: sum(1 for i in (items or []) if i.get("severity") == "must") >= 15,
    },
    "eagle_eye": {
        "name": "火眼金睛",
        "icon": "🔍",
        ...
        "quote": "鸬鹚的荣耀",
        "detect": lambda stats, items, **kw: any(
            any(k in (i.get("issue", "") + i.get("suggestion", "")) for k in ["类型不一致", "类型冲突", "datetime", "date", "varchar", "bigint", "tinyint"])
            for i in (items or [])
        ),
    },
    "perfect_sync": {
        ...
        "quote": "啄木鸟与 PM 心有灵犀",
        "detect": lambda stats, items, **kw: len(items or []) > 0 and kw.get("rejected_count", 0) == 0,
    },
    "rebel": {
        "name": "逆鳞",
        "icon": "⚔️",
        ...
        "detect": lambda stats, items, **kw: kw.get("rejected_count", 0) >= 5,
    },
    "rich_forest": {
        "name": "知识富矿",
        "icon": "🌳",
        ...
        "quote": "这片森林够大了",
        "detect": lambda stats, items, **kw: len(glob_module.glob(os.path.join(kw.get("wiki_path", ""), "*.md"))) >= 50 if kw.get("wiki_path") else False,
    },
    "pigeon_home": {
        "name": "信鸽归巢",
        "icon": "🕊️",
        ...
        "quote": "反馈的种子已播下",
        "detect": lambda stats, items, **kw: False,  # 手动触发
    },
    "full_flock": {
        "name": "全家福",
        "icon": "🐦",
        ...
        "quote": "鸟群出击，无一遗漏",
        "detect": lambda stats, items, **kw: len(set(i.get("dimension", "") for i in (items or []) if i.get("dimension"))) >= 4,
    },
    "typo_hunter": {
        "name": "笔误猎手",
        "icon": "🎯",
        ...
        "quote": "连打字错误都逃不过",
        "detect": lambda stats, items, **kw: any(
            any(k in i.get("issue", "") for k in ["笔误", "自相矛盾", "方向相同", "拼写"])
            for i in (items or [])
        ),
    },
    "blind_spot_pioneer": {
        ...
        "quote": "勇敢面对未知",
        "detect": lambda stats, items, **kw: kw.get("knowledge_gaps", 0) >= 3,
    },
    "ten_reviews": {
        "name": "十啄之功",
        "icon": "🏅",
        "desc": "累计完成 10 次评审",
        "quote": "量变引起质变",
        "detect": lambda stats, items, **kw: stats.get("total_reviews", 0) >= 10,
    },
}
...
def check_achievements(wiki_path, review_items=None, rejected_count=0, knowledge_gaps=0):
    ...
    for key, ach in ACHIEVEMENTS.items():
        if key in data["unlocked"]:
            continue
        detect = ach.get("detect")
        if detect and detect(
            data["stats"], review_items,
