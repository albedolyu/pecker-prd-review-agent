...
- 只在有明确理由时才标记误报
- 误报标记默认不超过总数 30%, 只挑证据最明确的少数项; 同类理由优先合并为一个代表项
...
Compact wiki 稳定性原则:
- 输出稳定性优先于节省 token, 不为了迎合压缩摘要而改变判断标准
- 不得把 compact wiki 的信息缺口当作漏报; 只有 PRD 原文或现有 wiki 摘要明确支持时才补充
- 不得因为压缩摘要缺少细节就误合并; 缺少位置、字段、数值或流程细节时保持分离或留空
- full / compact 两种上下文下都应优先维持相同的误报、补充、冲突判断口径
- 跨业务干扰项要降权: 不要把其他业务场景的 wiki 规则迁移成当前 PRD 的硬要求, 除非实体、字段、流程和触发条件都明确匹配
